...
RunAction::RunAction() : G4UserRunAction()
{
  ...
  // Add an ntuple for the additional homogeneous calorimeter
  std::string firstcolumnName = "Homogeneous Layer";
  analysisManager->CreateNtupleDColumn( firstcolumnName );

  for ( unsigned int layerIndex = 1; layerIndex <= layerNumber; ++layerIndex )
  {
    std::string columnName = "Sampling Layer" + std::to_string( layerIndex );
