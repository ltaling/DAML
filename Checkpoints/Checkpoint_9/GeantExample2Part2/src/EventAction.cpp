...
void EventAction::EndOfEventAction( const G4Event* )
{
  ...
  std::cout << "Positrons produced from pair production: " << m_ppPositrons.size() << std::endl;
}
...
void EventAction::RecordParticleData( const G4Step* step )
{
  ...
  if ( track->GetCreatorProcess() )
  {
    ...
    // A check so that only photon created positrons are counted in ppPositron
    if ( creator == "conv" && particle == "e+")
    {
	  // Use a set so that track IDs are only stored once
      m_ppPositrons.insert( track->GetTrackID() );
